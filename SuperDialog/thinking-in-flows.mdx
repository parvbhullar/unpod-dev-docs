...
```
[greet] -> (user_ready) -> [collect_name] -> (name_given) -> [confirm] -> (confirmed) -> [done]
```
...
```
[greet]
  -> user wants to book    -> [collect: name, doctor, date, time]
  -> user wants to reschedule -> [collect: booking_ref, new_date]
  -> [confirm]
  -> [done]
```
