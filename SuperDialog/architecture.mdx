...
DialogMachine.turn(text)
    ...
    ├─ Build prompt (system + history + node instructions)
    ├─ Call LLM (model URI resolver → provider)
    ├─ Execute tool calls (Python / HTTP / MCP)
    ├─ Update slots, evaluate edge conditions
    ├─ Advance to next node
    │
    ▼
Turn(text, tool_calls, metadata)
    │
    ▼
Host (LiveKit / PipeCat / FastAPI / WSS / CLI)
```
