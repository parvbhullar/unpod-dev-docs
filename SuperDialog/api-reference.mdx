...
Set `traversal_dir` to a directory path and the machine writes a timestamped JSON file on each completed session recording every node visited, every turn, and all collected slot values. Useful for debugging flows, building eval datasets, and production auditing.
