...
```
unpod          unpod           SuperDialog          Open-Source
  |                   |                    |               CPAAS Platform
  |                   |                    |                    |
  v                   v                    v                    v
Communication     Voice Stack         Conversation         Agent Management
  Infra           (STT/TTS)           Framework            (Platform)
```
...
Caller
  |
  v
[ Unpod Speech Platform ]   <- phone number + voice stack
  |  transcribed text
  v
[ SuperDialog ]             <- conversation framework (your logic)
  |  text reply
  v
[ Unpod Speech Platform ]   <- synthesise + stream back
  |
  v
