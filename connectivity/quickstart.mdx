...
<Steps>
  ...
  <Step title="See the numbers available to you">
    ...
    See [List Numbers](/connectivity/numbers/list-numbers).
  </Step>
  ...
  <Step title="Create a trunk (your SIP carrier credential)">
    ...
    The response returns the new trunk `id` (e.g. `21`). Copy it. See [Create Trunk](/connectivity/trunks/create-trunk).
  </Step>
  ...
  <Step title="Map a number to the trunk">
    ...
    inbound calls to, plus the accepted source IPs. See [Attach Numbers to Trunk](/connectivity/trunks/attach-numbers).
  </Step>
  ...
  <Step title="Confirm the lifecycle">
    ...
    See [Telephony Overview](/connectivity/calls/overview).
  </Step>
</Steps>
...
<CardGroup cols={2}>
  <Card title="Detach numbers" icon="unlink" href="/connectivity/trunks/detach-numbers">
    Step 1 — unmap numbers from the trunk, returning them to `NOT_ASSIGNED`.
  </Card>
  <Card title="Delete trunk" icon="trash-2" href="/connectivity/trunks/delete-trunk">
