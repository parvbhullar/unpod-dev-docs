...
The controls above run **inside** your entrypoint, on `ctx.session`. You can also
act on a live session **from outside** it - e.g. from your backend or an ops tool
- using the management SDK. These target the session by ID and are useful for
supervisor handoffs, bulk control, or reacting to events elsewhere in your stack.
...
client must be configured to reach it (see the SDK client `orchestrator_base_url`
option). The in-call `ctx.session.transfer_to_*` helpers remain the simplest way
to transfer from within your dialog logic.
</Note>
