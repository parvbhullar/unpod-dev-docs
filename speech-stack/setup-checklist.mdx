...
| `UNPOD_BASE_URL` | No | The one endpoint (default `api.unpod.ai`): REST derives `https://<host>/platform`, the runner derives `wss://<host>` |
| `UNPOD_SERVICE_BASE_URL` | No | Override for Management REST only, e.g. `http://localhost:8000/platform` for local service testing |
| `UNPOD_ORCHESTRATOR_URL` | No | Override for the runner WebSocket only |
