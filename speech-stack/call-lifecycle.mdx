...
your agent and the caller are two participants in a LiveKit room, and Unpod
bridges the audio between them while streaming transcripts to your
...
<Steps>
  ...
  <Step title="Your agent joins and talks">
    Your `AgentRunner` is connected to the same room. Audio flows; transcripts
    stream to your dialog logic. The call row moves to `active`, then
    `completed` on hangup.
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Your agent joins the live room">
    Unpod connects your `AgentRunner` to the room the caller is already in.
    Audio and transcripts start immediately.
  </Step>
</Steps>
