...
- A configured agent with a voice profile - see [Agents](/speech-stack/agents)
- A phone number attached to that agent - see [Numbers](/speech-stack/numbers)
...
async def main():
    async with AsyncClient() as client:
        call = await client.calls.create(
            agent_id="my-agent",         # or pipe_id= for older code
            to_number="+14155550100",    # E.164 format
            from_number="+18005551234",  # optional if a number is attached to the agent
...
| `agent_id` | `str \| None` | Conditional | **Preferred.** Agent to route the call to. Takes priority over `pipe_id`; the platform resolves the agent's voice row server-side. `agent=` is an alias kept for older callers |
| `pipe_id` | `str \| None` | Conditional | The deprecated spelling. Provide `agent_id` **or** `pipe_id` (`agent_id` wins if both are given) |
| `from_number` | `str` | No | Caller ID. If omitted, Unpod uses a number attached to the agent |
...
| `pipe_id` | `str` | The agent-voice row that handled the call |
...
async def main():
    async with AsyncClient() as client:
        # All outbound calls for one Speech Pipe row
...
| `pipe_id` | `pipe_...` | Filter by agent-voice row |
...
async def process_callback_queue(queue: list[dict]) -> None:
    async with AsyncClient() as client:
        for request in queue:
            # Capacity is handled for you: the orchestrator rejects a dispatch
            # when the agent's runners are full
            call = await client.calls.create(
                agent_id="my-agent",
