...
title: 'Hooks & Events'
icon: 'webhook'
description: 'React to the call events the SDK actually emits: turns, interruptions, lifecycle changes, and observability hooks.'
---
...
- **Session-level** - fire on events within a single call (e.g. per-turn logic, call cleanup, metrics)
...
### Registrable but never fired

`session.on(...)` validates the event name against the registry, so these four
**register without error and are then never called**. Nothing in the SDK emits
them:

| Event | Status |
|---|---|
| `user_partial` | No interim-transcription frame reaches the SDK. Use `user_turn` (final transcript). |
| `tool_call` | Not emitted. Instrument tool calls inside your own brain/adapter. |
| `tool_result` | Not emitted. Same - instrument it yourself. |
| `silence` | Not emitted. Implement a watchdog with `asyncio.wait_for` around your turn logic (see the pattern [below](#silence-detection-with-timeout)). |

<Warning>
Do not build a feature on these. A handler registered for one of them is dead
code, and because registration succeeds there is no error to tell you so.
</Warning>
...
<Warning>
**A raising hook ends the call.** The registry logs the offending handler's
name and then **re-raises**, so the exception propagates out of
`session.run()`, terminates the call, and bills it to your entrypoint as
`failed`. Later handlers for the same event never run.

Wrap anything that can fail - a CRM lookup, an analytics POST - in its own
`try/except` inside the hook:

```python
@ctx.session.on("user_turn")
async def log_turn(text: str) -> None:
    try:
        await analytics.post(text)
    except Exception:
        log.exception("analytics failed")   # never let it reach session.run()
```
</Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="AgentRunner & Sessions" icon="sliders-horizontal" href="/speech-stack/agent-runner">
    Use say, transfer, and end on live calls.
  </Card>
</CardGroup>
