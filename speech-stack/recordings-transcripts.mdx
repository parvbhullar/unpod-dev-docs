...
icon: 'file-music'
description: 'Retrieve call audio and per-turn transcripts after calls end.'
---
...
<Warning>
**There is no live pause/resume.** `ctx.session.recording.pause()` / `.resume()`
exist on the `Session` object but are not implemented on the bridge protocol -
calling them makes the agent **speak the sentinel aloud** to the caller. To keep
sensitive audio out of a recording, run the PII step on an agent created with
`recording=False`. See
[Recording control](/speech-stack/agent-runner#recording-control).
</Warning>
