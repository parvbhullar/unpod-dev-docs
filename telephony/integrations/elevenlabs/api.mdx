...
## Next: configure the ElevenLabs side

The Unpod side is done. In ElevenLabs, import the number **from SIP trunk** (outbound
Address = the origin endpoint above), then attach your Conversational AI agent. That's
ElevenLabs' own setup — follow their guide.

<Card title="ElevenLabs SIP trunking docs" icon="arrow-up-right-from-square" href="https://elevenlabs.io/docs/eleven-agents/phone-numbers/sip-trunking">
  Import the number from SIP trunk, set transport/encryption, and attach the agent.
</Card>
