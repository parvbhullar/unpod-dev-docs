...
Audit a recorded session or run a synthetic eval.

```bash
superdialog eval --flow kyc.json --traversal session.json
```

Outputs an audit / comparison report. (For playbook persona evals from Python,
see `run_eval` in the [API Reference](/superdialog/api-reference#replay-and-the-eval-bridge).)
