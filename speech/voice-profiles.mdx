...
icon: 'waveform'
description: 'Browse and select the speech stack for your voice agents.'
---
...
When you attach a voice profile to an agent, every call that agent handles uses that configuration automatically — including failover if a provider goes down.

<Frame>
  <img src="/images/diagrams/voice-profile-pipeline.png" alt="Voice profile pipeline diagram showing caller speech passing through STT, dialog machine text handling, TTS plus voice, and caller playback." />
</Frame>
...
|---|---|---|
| `profile_id` | `str` | Profile ID to pass when creating an agent |
...
Pass `profile_id` directly when creating or updating an agent:
...
async def main():
    async with AsyncClient(api_key="sk-...") as client:
        ...
        print("Updated voice profile:", agent.voice_profile_id)
