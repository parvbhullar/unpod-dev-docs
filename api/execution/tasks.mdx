...
const getTasksBySpace = async (spaceToken, page = 1, pageSize = 20) => {
  const response = await axios.get(
    `https://unpod.ai/api/v2/platform/spaces/${spaceToken}/tasks/?page=${page}&page_size=${pageSize}`,
    { headers }
  );
  console.log(`Total tasks: ${response.data.count}`);
  return response.data.data;
...
```bash cURL
# Get tasks by agent handle
curl -X GET "https://unpod.ai/api/v2/platform/agents/sales-agent-v1/tasks/" \
...
# Create a task in a space
curl -X POST "https://unpod.ai/api/v2/platform/spaces/your-space-token/tasks/" \
  -H "Authorization: Token your-api-token" \
  -H "Content-Type: application/json" \
  -d '{
