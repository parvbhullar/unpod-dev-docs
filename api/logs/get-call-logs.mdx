...
const getCallLogs = async (filters = {}) => {
  ...
  const response = await axios.get(
    'https://unpod.ai/api/v2/platform/call-logs/',
    { headers, params }
  );
...
def get_call_logs(page: int = 1, page_size: int = 20, **filters):
    ...
    url = 'https://unpod.ai/api/v2/platform/call-logs/'
...
```bash cURL
# Get call logs with filters
curl -X GET "https://unpod.ai/api/v2/platform/call-logs/?page=1&page_size=20&call_type=outbound&call_status=completed" \
  -H "Org-Handle: your-org-handle" \
  -H "Authorization: Token your-api-token"

# Get call logs for a specific date range
curl -X GET "https://unpod.ai/api/v2/platform/call-logs/?start_date=2026-02-01&end_date=2026-02-07" \
  -H "Org-Handle: your-org-handle" \
  -H "Authorization: Token your-api-token"
```
