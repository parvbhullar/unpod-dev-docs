...
- `UNPOD_PLATFORM_TOKEN` plus `UNPOD_ORG_HANDLE` in the environment - needed
  for `voice_profiles.list()` during one-time setup
...
Provisioning uses the **Management API** - the REST half of the
[SDK](https://github.com/unpod-ai/unpod-python-sdk), reached through
`AsyncClient`. This setup script starts with `voice_profiles.list()`, so run it
with `UNPOD_PLATFORM_TOKEN` plus `UNPOD_ORG_HANDLE`. The later REST calls on
this page also work with `UNPOD_API_KEY`, and the runner still requires that
API key for its own WebSocket connection. The REST endpoint derives from
`UNPOD_BASE_URL` (default `api.unpod.ai` → `https://<host>/platform`).
...
async def setup() -> None:
    async with AsyncClient() as client:
        ...
        #    The flat `languages` field is usually empty - the codes arrive in
        #    the nested voice/transcriber dicts. See the Voice Profiles page for
        #    a `speaks(profile, code)` helper that checks both.
        ...
        #    The pool reports NOT_ASSIGNED; a supervoice row reports available.
        #    Do not filter on status="active" (no plane emits it) or on
        #    pipe_id is None (pool rows carry no pipe_id).
        numbers = await client.numbers.list()
        free = [n for n in numbers if (n.status or "").lower() in {"not_assigned", "available"}]
        if not free:
        ...
        number = await client.numbers.attach(
            free[0].number_id, "browser-playground", number=free[0].number)
...
3. **`numbers.list()`** - find a number in your account with nothing attached.
   Filter on `status`, not on `pipe_id` - see
...
<Note>
Step 1 (`voice_profiles.list()`) reads the **platform** plane and needs
`UNPOD_PLATFORM_TOKEN` plus `UNPOD_ORG_HANDLE`; a bare `UNPOD_API_KEY` is
refused there. Steps 2-4 work with either credential. See the [credential
matrix](/speech-stack/setup-checklist#which-credential-goes-where).
</Note>
