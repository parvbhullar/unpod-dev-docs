...
- **A registered trunk.** Numbers come from a
  [trunk](/get-started/core-concepts#trunk) - the SIP connection between a
  telephony provider and Unpod. Without one you have no numbers to attach, and
  the setup below stops at step 4.

<Warning>
**Register a trunk before you start.** Run `client.trunks.create(...)` once - see
[Trunks](/speech-stack/numbers#trunks). You configure a trunk once; after that
you work with numbers, not SIP.
</Warning>
...
async def setup() -> None:
    async with AsyncClient() as client:
        ...
        available = await client.numbers.list(status="active")
        if not available:
...
3. **`numbers.sync()`** - pull numbers in from your registered trunks.
4. **`numbers.attach()`** - route one number to the pipe.

If step 4 finds nothing, you have no trunk - see the prerequisite above.
