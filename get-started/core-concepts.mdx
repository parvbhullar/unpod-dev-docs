...
flowchart LR
    ...
    subgraph Unpod["Unpod infrastructure (audio)"]
        Number
        Trunk
        Bridge
        STT["STT / TTS<br/>voice profile"]
    end

    Bridge <-->|audio| STT
    Bridge <-->|text over WSS| Runner

    subgraph Yours["Your process (text)"]
        Runner["AgentRunner"]
        Runner --> Agent["Agent / Brain"]
        Agent -. optional .-> SD["SuperDialog<br/>DialogMachine"]
    end
```

Read it as: a **Caller** dials a **Number**, which routes over a **Trunk** into
Unpod. Unpod transcribes the audio with the call's **voice profile** (STT) and
sends plain text across the **Bridge** over a WebSocket to your **AgentRunner**.
Your runner hands each turn to your **Agent** (your brain). Your reply text
crosses back over the Bridge, Unpod synthesises it (TTS), and the caller hears
it. SuperDialog is one option for the brain - powerful, but optional.
...
not SIP. (A dedicated Trunks page is coming; for now trunks are set up via
...
It is Unpod-internal infrastructure - you do not configure it, but the term
...
[SDK Setup](/speech-stack/sdk-setup).
...
brain. See [Session Controls](/speech-stack/session-controls).
...
hosted Platform or its v2 API - the voice SDK does not require one.
...
| **`pipe_id`** | One Speech Pipe in the Control Plane | A UUID **Unpod assigns** when you create the pipe. Used in REST calls (`numbers.attach`, `calls.create`). |
| **Space token** | A Platform workspace | A token from the Platform's Spaces API. Used only in hosted Platform / v2 API URLs, not in the voice SDK. |
...
<CardGroup cols={2}>
  ...
  <Card title="Speech Stack" icon="layers" href="/speech-stack/sdk-setup">
