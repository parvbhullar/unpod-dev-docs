...
over WebSocket, advertises capacity, and under the default `dial_out`
transport **dials out** to a per-call bridge when work is assigned. The older
`serve` transport flips that around and has Unpod dial into your runner
instead. For each call it builds a `CallContext` and invokes your entrypoint.
...
controls (`say()`, `transfer_to_human()`, `end()`), hooks, metrics, and the
`dialog_machine` slot where you plug in your brain. Recording is configured on
the agent itself; live pause/resume controls are not implemented. Calling
...
credentials from the environment, preferring `UNPOD_PLATFORM_TOKEN` over
`UNPOD_API_KEY`.

```python
from unpod import AsyncClient, Runner

client = AsyncClient()   # UNPOD_PLATFORM_TOKEN first, else UNPOD_API_KEY
await client.agents.voice.create("support-bot", brain=Runner(), name="Support")
```

<Warning>
The sync `Client` **raises** if you call it from inside a running event loop:
`RuntimeError("Client cannot be used from a running event loop; use AsyncClient")`.
Use it from scripts and sync web handlers; use `AsyncClient` everywhere `async`
is already in play - including inside an `AgentRunner` entrypoint.
</Warning>

A few resources need a platform token specifically, and some are unreachable in
proxy mode. See [Client, Auth & Planes](/speech-stack/client-auth).
