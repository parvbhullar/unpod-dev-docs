---
title: 'Unpod Open Source'
sidebarTitle: 'Introduction'
description: 'Getting started with Unpod - Open-source'
icon: 'book'
---

##  AI Native Communication Platform

Unpod is an open-source communication platform for creating AI agents with dedicated phone numbers. Build agents that handle incoming calls and messages, filter communications intelligently, and deliver actionable insights — all while integrating with your existing business tools.


## Demo: See Unpod In Action

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.02)'}}>
  <iframe
     src="https://drive.google.com/file/d/1-L7xDzYywO-IQupA5Kx8TK3xezZmXAKb/preview"
    title="Unpod Demo Video"
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', border: 0}}
    allow="autoplay; fullscreen"
    allowFullScreen
  />
</div>

### Platform Base URLs

Unpod provides two different portals depending on your requirements - the main application interface and the developer environment. Below are the base URLs and what each one is used for:

### Unpod AI Platform (User Dashboard)

**URL:** https://unpod.ai/

**Purpose:**
Main user-facing platform where organizations, teams log in.
Used for managing workspaces, telephony setup, user dashboards, analytics, and day to day operations.

### Unpod Developer Platform

**URL:** https://unpod.ai/

**Purpose:**
This portal is meant for developers, integrators, and technical teams.
Contains developer tools, API access, integrations, telephony configuration, and testing utilities.


## Choose Your Path

<CardGroup cols={2}>

<Card title="Business Users" icon="building" href="/platform/introduction">
  **Best for:** Non-technical users, business teams, customer support managers

  **Prerequisites:**
  - An active email account
  - Modern web browser (Chrome, Firefox, Safari, or Edge)
  - Internet connection

  **What you get:**
  - No-code AI agent builder
  - Pre-built templates
  - Visual dashboard
  - Team collaboration tools

  **Get started at** [unpod.ai](https://unpod.ai)
</Card>

<Card title="Developers" icon="code" href="/platform/self-hosting/quickstart">
  **Best for:** Developers, technical teams, system integrators

  **Prerequisites:**
  - Node.js v20+ / npm v10+
  - Python 3.11+
  - Docker and Docker Compose
  - Git

  **What you get:**
  - Full API access
  - Custom telephony configuration
  - Advanced agent programming
  - Developer tools and SDKs

  **Get started at** [github.com/parvbhullar/unpod](https://github.com/geneffic/unpod)
</Card>

</CardGroup>

No credit card required. Free 14-day trial. Cancel anytime.

## How Unpod Works?

With Unpod, you can setup three basic components:

<CardGroup cols={3}>
  <Card title="Setup Space" icon="layout-grid">
    A dedicated space that helps to manage your emails, contacts, and documents.
  </Card>
  <Card title="Setup Agent" icon="bot">
    A custom AI agent for all your needs like customer support and sales support.
  </Card>
  <Card title="Setup Telephony" icon="phone">
    Add a number, link a voice infra provider, complete KYC, and start calling.
  </Card>
</CardGroup>

## Features

<CardGroup cols={2}>
  <Card title="AI Voice Agents" icon="bot">
    Build conversational voice agents powered by LLMs with customizable personality, knowledge, and tools.
  </Card>
  <Card title="Multi-Channel" icon="messages-square">
    Voice calls, WhatsApp, and email through a unified agent interface.
  </Card>
  <Card title="Real-Time Voice Pipeline" icon="bolt">
    Sub-second latency using LiveKit, Pipecat, and streaming TTS/STT.
  </Card>
  <Card title="Agent Studio" icon="bot">
    Visual no-code builder for configuring agent behavior, prompts, and workflows.
  </Card>
  <Card title="Knowledge Base" icon="book">
    Upload documents and data sources for RAG-powered agent responses.
  </Card>
  <Card title="Multi-Tenant Workspaces" icon="building">
    Organizations, teams, RBAC, and shared spaces.
  </Card>
  <Card title="Telephony Integration" icon="phone">
    Dedicated phone numbers with SIP trunking and call routing.
  </Card>
  <Card title="Call Analytics" icon="chart-line">
    Real-time dashboards, conversation logs, and performance metrics.
  </Card>
  <Card title="Workflow Automation" icon="settings">
    Trigger actions (scheduling, CRM updates, notifications) from conversations.
  </Card>
  <Card title="Desktop App" icon="monitor">
    Native cross-platform desktop client built with Tauri.
  </Card>
</CardGroup>


## Use Cases

<CardGroup cols={2}>
  <Card title="Customer Support" icon="headset">
    Automate inbound calls with AI agents and escalate to humans when needed.
  </Card>
  <Card title="Sales & Lead Qualification" icon="phone-call">
    Make outbound sales calls and schedule appointments with prospects.
  </Card>
  <Card title="Appointment Scheduling" icon="calendar-check">
    Handle booking details and schedule appointments after confirming availability.
  </Card>
  <Card title="E-commerce" icon="shopping-cart">
    Customer support and personalized product recommendations.
  </Card>
</CardGroup>

## Why Unpod?

- **Open Source** — Full control over agent personality, functionality, themes, and data models
- **Flexible** — Run simple flows (number + provider) or advanced omni-channel workflows
- **Reliable** — 99.99% uptime with automatic failover
- **Secure** — Enterprise-grade security and compliance
- **Global Reach** — Communicate with users worldwide

## Key Capabilities

- Real time communication - Natural voice conversations.
- Omni Channel Support - Voice, WhatsApp, and Email
- AI Driven Automation - Context aware responses and workflow automation
- Integration ready - Connect to CRMs, ERPs, and other business tools
- Flexible routing - Use numbers, providers, and bridges to control traffic